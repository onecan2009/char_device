...
#define DMA_TX_SIZE 5
static int dma_txsize = DMA_TX_SIZE;
module_param(dma_txsize, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(dma_txsize, "Number of descriptors in the TX list");

#define DMA_RX_SIZE 5
...
#endif
static void stmmac_direct_dma_interrupt(struct stmmac_priv *priv);
 ...
 */
 ...
 extern Device_t *g_Device;//terry 2015-12-10
...
static int stmmac_open(struct net_device *dev)
{
	...
	int ret;
/****terry 2015-12-8***************/
	int cpu = 3;//bind cpu 3 by 2016-11-21
...
/******terry 2015-12-10********************************/	
	...
	disable_irq(98);//disable timer0 interrupt
/*****************************************************/
	spin_lock(&priv->tx_lock);
	...
	spin_unlock(&priv->tx_lock);
	enable_irq(98);//terry 2015-12-10 enable timer0 interrupt
	return NETDEV_TX_OK;
}
 ...
 */
static inline void stmmac_rx_refill(struct stmmac_priv *priv)			// 重新分配skbuff，并初始化对应描述符
{
	unsigned int rxsize = priv->dma_rx_size;
	int bfsize = priv->dma_buf_sz;

	for (; priv->cur_rx - priv->dirty_rx > 0; priv->dirty_rx++) {			// 遍历本设备中所有空的skbuff
...
static void Stmmac_get_time(int64_t *Counter)
{
	struct timeval tv;

	do_gettimeofday(&tv);									// 获取当前时间
	*Counter = tv.tv_sec * (uint64_t)1000000 + tv.tv_usec;		// 转换成us

	return;
}

#define MAX_RX_DELAY_US	200

unsigned int stmmac_peek_packet(unsigned char *pbData, unsigned int length, int limit)
{
	...
	unsigned int dma_status, int_status = 0;
	struct ethhdr* ethhdr = NULL;
	int coe;
	int64_t _start_time, _end_time;
	...
	coe = priv->plat->rx_coe;				// 是否支持coe模式，在STMMAC_RX_COE_TYPE1模式下，会在数据包末尾添加两个效验字

	Stmmac_get_time(&_start_time);
	_end_time = _start_time + MAX_RX_DELAY_US;

	do{															// 超时等待接受中断
		dma_status = readl(priv->ioaddr + DMA_STATUS);
		/* TX/RX NORMAL interrupts */
		if (likely(dma_status & DMA_STATUS_NIS)) {				// DMA发生中断
			if (likely(dma_status & DMA_STATUS_RI)) {
				unsigned int value = readl(priv->ioaddr  + DMA_INTR_ENA);
				if (likely(value & DMA_INTR_ENA_RIE)) {
					int_status |= handle_rx;
		...
		Stmmac_get_time(&_start_time);				//	更新时间
	}while(_start_time < _end_time);


	rxsize = priv->dma_rx_size;
	entry = priv->cur_rx % rxsize;			// 获取最新描述符
	...
	while (count < limit) {
		...
		if (priv->hw->desc->get_rx_owner(p)){		// 若描述符被DMA控制
			break;
		}

		next_entry = (priv->cur_rx + 1) % rxsize;		// 预先取数
		if (priv->extend_desc){
		...
		status = priv->hw->desc->rx_status(&priv->dev->stats,		// 根据描述符获取rx数据帧状
						   &priv->xstats, p);
		if ((priv->extend_desc) && (priv->hw->desc->rx_extended_status)){	// 若支持增强描述符
...
//			}
			...
			frame_len = priv->hw->desc->get_rx_frame_len(p, coe);		// 从描述表中获取数据长

			/* ACS is set; GMAC core strips PAD/FCS for IEEE 802.3
			 * Type frames (LLC/LLC-SNAP)
			 */
			if (unlikely(status != llc_snap)){							// 不支持该功能
				frame_len -= ETH_FCS_LEN;
			}

			skb = priv->rx_skbuff[entry];
			if (unlikely(!skb)) {										// 若skb指针无效
...
//			stmmac_get_rx_hwtstamp(priv, entry, skb);				// 该设备不支持IEEE1588

			skb_put(skb, frame_len);
//			dma_unmap_single(priv->device,							// 不允许取消映
//					 priv->rx_skbuff_dma[entry],
//					 priv->dma_buf_sz, DMA_FROM_DEVICE);
			skb->protocol = eth_type_trans(skb, priv->dev);
/***********************************************************************/
			//unsigned int i;
                   ethhdr = eth_hdr(skb);//gy add 20151129
			//printk("stamac:0x%x\n", *(unsigned int*)(ethhdr->h_source));
			if (likely(skb->protocol  == 0xa488 && *(unsigned int*)(ethhdr->h_source) == 0x02))//APRD指令
			{
				count++;							// 计算收取ecat数据包个

				if(length >= frame_len-14){			// 若接收缓存长度大于实际数据包长度，进行拷贝数
					memcpy(pbData, skb->data, frame_len-14);
				}
...
#ifdef PUNGO_DRIVER
        struct ethhdr* ethhdr = NULL;//gy add 20151129
#endif
...
static void stmmac_direct_dma_interrupt(struct stmmac_priv *priv)
{
	...
	status = priv->hw->dma->dma_interrupt(priv->ioaddr, &priv->xstats);	// 统计中断数量
	if (likely((status & handle_rx)) || (status & handle_tx)) {		// 若有接收或者发送中断，执行接收发送数据中断
		stmmac_disable_dma_irq(priv);				// 关闭DMA中断
		stmmac_tx_clean(priv);						// 清除发送
		stmmac_rx(priv, 64);						// 接收数据
...
static irqreturn_t stmmac_interrupt(int irq, void *dev_id)
{
	...
	stmmac_direct_dma_interrupt(priv);
	return IRQ_HANDLED;
}
...
static int stmmac_hw_init(struct stmmac_priv *priv)
{
	...
	if (priv->plat->has_gmac) {
        ...
        printk("dwmac1000 ...,priv->ioaddr is 0x%x\n",priv->ioaddr);
	} else {
				     ...
				     void __iomem *addr)
{
	...
	if (!ndev)
        ...
        memmove(ndev->name, "eth-gfd", IFNAMSIZ);//gengyang 2015-12-5
...
int stmmac_init(void)
{
	...
	//misc_register(&rtl8211e_device);//terry 2015-9-22
	return 0;
err_pci:
...
void stmmac_exit(void)
{
	...
	//misc_deregister(&rtl8211e_device);//terry 2015-9-22
}
#else
